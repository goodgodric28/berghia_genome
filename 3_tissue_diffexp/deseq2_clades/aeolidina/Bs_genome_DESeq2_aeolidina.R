####################################
# Bs_genome_DESeq2_aeolidina.R
# Written by: Jessica A. Goodheart
# Last Updated: 2 August 2022
# Purpose: To analyze differential expression data from Berghia tissues in genes specific to Aeolidina.
...
directory <- "[PATH TO]/tissue_diffexp/deseq2_clades/aeolidina/"
...
outputPrefix <- "Bs_tissues_aeolidina_DESeq2"

# Pull in Berghia gene ids for Aeolidina-specific genes from kinfin analysis
aeolidina_novel_genes <- scan("Aeolidina_only_genes.txt", what="", sep="\n")

# Subset results to only include Aeolidina-specific genes and order results by padj value (most significant to least)
res <- resdata[resdata$gene %in% aeolidina_novel_genes, ]
res <- res[order(res$padj),]

# Send subsetted and normalized counts to tab delimited file
dds_filt <- dds[aeolidina_novel_genes, ]
...
library("gplots")
distsRL <- dist(t(assay(rld[aeolidina_novel_genes, ])))
...
rv <- rowVars(assay(rld[aeolidina_novel_genes, ]))
...
heatmap.2(assay(rld[aeolidina_novel_genes, ]), col=my_palette,
...
# Pull out annotation data for tissue-specific Aeolidina genes
### Distal Ceras Only ###
...
dc_label_summary <- sort(summary(as.factor(dc_only_annot$label)), decreasing=TRUE)
...
dc_goterm_summary <- sort(summary(as.factor(dc_only_annot$go_term)), decreasing=TRUE)
...
# Sort annotations by number of times present
ot_label_summary <- sort(summary(as.factor(ot_only_annot$label)), decreasing=TRUE)
...
# Sort go terms by number of times present
ot_goterm_summary <- sort(summary(as.factor(ot_only_annot$go_term)), decreasing=TRUE)
...
# Sort annotations by number of times present
rh_label_summary <- sort(summary(as.factor(rh_only_annot$label)), decreasing=TRUE)
...
# Sort go terms by number of times present
rh_goterm_summary <- sort(summary(as.factor(rh_only_annot$go_term)), decreasing=TRUE)
...
# Sort annotations by number of times present
br_label_summary <- sort(summary(as.factor(br_only_annot$label)), decreasing=TRUE)
...
# Sort go terms by number of times present
br_goterm_summary <- sort(summary(as.factor(br_only_annot$go_term)), decreasing=TRUE)
...
# Sort annotations by number of times present
fo_label_summary <- sort(summary(as.factor(fo_only_annot$label)), decreasing=TRUE)
...
# Sort go terms by number of times present
fo_goterm_summary <- sort(summary(as.factor(fo_only_annot$go_term)), decreasing=TRUE)
...
# Sort annotations by number of times present
ta_label_summary <- sort(summary(as.factor(ta_only_annot$label)), decreasing=TRUE)
...
# Sort go terms by number of times present
ta_goterm_summary <- sort(summary(as.factor(ta_only_annot$go_term)), decreasing=TRUE)
...
# Sort annotations by number of times present
pc_label_summary <- sort(summary(as.factor(pc_only_annot$label)), decreasing=TRUE)
...
# Sort go terms by number of times present
pc_goterm_summary <- sort(summary(as.factor(pc_only_annot$go_term)), decreasing=TRUE)
