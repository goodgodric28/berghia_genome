####################################
# Bs_genome_DESeq2_nudibranchia.R
# Written by: Jessica A. Goodheart
# Last Updated: 30 May 2023
# Purpose: To analyze differential expression data from Berghia tissues in genes specific to Nudibranchia.
...
directory <- "[PATH TO]/tissue_diffexp/deseq2_clades/nudibranchia/"
...
outputPrefix <- "Bs_tissues_nudibranchia_DESeq2"

# Pull in Berghia gene ids for Nudibranchia-specific genes from kinfin analysis
nudibranchia_novel_genes <- scan("Nudibranchia_only_genes.txt", what="", sep="\n")

# Subset results to only include Nudibranchia-specific genes and order results by padj value (most significant to least)
res <- resdata[resdata$gene %in% nudibranchia_novel_genes, ]
res <- res[order(res$padj),]

# Send subsetted and normalized counts to tab delimited file
dds_filt <- dds[nudibranchia_novel_genes, ]
...
library("gplots")
distsRL <- dist(t(assay(rld[nudibranchia_novel_genes, ])))
...
rv <- rowVars(assay(rld[nudibranchia_novel_genes, ]))
...
heatmap.2(assay(rld[nudibranchia_novel_genes, ]), col=my_palette,
...
# Pull out annotation data for tissue-specific Nudibranchia genes
### Distal Ceras Only ###
