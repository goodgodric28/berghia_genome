####################################
# Bs_genome_DESeq2_gastropoda.R
# Written by: Jessica A. Goodheart
# Last Updated: 30 May 2023
# Purpose: To analyze differential expression data from Berghia tissues in genes specific to Gastropoda.
...
directory <- "[PATH TO]/tissue_diffexp/deseq2_clades/gastropoda/"
...
outputPrefix <- "Bs_tissues_gastropoda_DESeq2"

# Pull in Berghia gene ids for Gastropoda-specific genes from kinfin analysis
gastropoda_novel_genes <- scan("Gastropoda_only_genes.txt", what="", sep="\n")

# Subset results to only include Gastropoda-specific genes and order results by padj value (most significant to least)
res <- resdata[resdata$gene %in% gastropoda_novel_genes, ]
res <- res[order(res$padj),]

# Send subsetted and normalized counts to tab delimited file
dds_filt <- dds[gastropoda_novel_genes, ]
...
library("gplots")
distsRL <- dist(t(assay(rld[gastropoda_novel_genes, ])))
...
rv <- rowVars(assay(rld[gastropoda_novel_genes, ]))
...
heatmap.2(assay(rld[gastropoda_novel_genes, ]), col=my_palette,
...
# Pull out annotation data for tissue-specific Gastropoda genes
### Distal Ceras Only ###
