...
directory <- "[PATH TO]/tissue_diffexp/deseq2_clades/all_genes/"
...
outputPrefix <- "Bs_tissues_allgenes_DESeq2"
...
png(file=paste0(outputPrefix, "-HEATMAP-top200.png"), width=1000, height=1000)
...
png(file=paste0(outputPrefix, "-HEATMAP-top500.png"), width=1000, height=1000)
heatmap.2(assay(rld)[select,], col=my_palette,
          scale="row", key=T, keysize=1, symkey=T,
          density.info="none", trace="none",
          cexCol=0.6, labRow=F,
...
dds_df.2 <- dds_df[rowSums(dds_df) > 10.0,]

# Dataset stats
statsfile<-file("summary_stats.txt")
writeLines(c(print(paste0("Total number of genes ", nrow(dds_df))),
             print(paste0("Number of genes w/ counts >10 across all tissues: ", nrow(dds_df.2))),
             print(paste0("Proportion of genes w/ counts >10 across all tissues: ", (nrow(dds_df.2)/nrow(dds_df))))), statsfile)
close(statsfile)

# Create empty venn diagram data frame
venn_df <- data.frame(matrix(ncol = 0, nrow = nrow(dds_df.2)))
rownames(venn_df) <- rownames(dds_df.2)

# Calculate average expression for each tissue type in new data frame
venn_df$DC <- (dds_df.2$`Distal Ceras 1` + dds_df.2$`Distal Ceras 2` + dds_df.2$`Distal Ceras 3`)/3
venn_df$PC <- (dds_df.2$`Proximal Ceras 1` + dds_df.2$`Proximal Ceras 2` + dds_df.2$`Proximal Ceras 3`)/3
venn_df$OT <- (dds_df.2$`Oral Tentacle 1` + dds_df.2$`Oral Tentacle 2` + dds_df.2$`Oral Tentacle 3`)/3
venn_df$RH <- (dds_df.2$`Rhinophore 1` + dds_df.2$`Rhinophore 2` + dds_df.2$`Rhinophore 3`)/3
venn_df$BR <- (dds_df.2$`Brain 7` + dds_df.2$`Brain 8`)/2
venn_df$TL <- (dds_df.2$`Tail 1` + dds_df.2$`Tail 2`)/2
venn_df$FO <- (dds_df.2$`Foot 1` + dds_df.2$`Foot 2`)/2
...
# Pull out annotation data for all tissue-specific genes
### Distal Ceras Only ###
