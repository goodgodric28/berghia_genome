####################################
# Bs_genome_DESeq2_mollusca.R
# Written by: Jessica A. Goodheart
# Last Updated: 30 May 2023
# Purpose: To analyze differential expression data from Berghia tissues in genes specific to Mollusca.
...
directory <- "[PATH TO]/tissue_diffexp/deseq2_clades/mollusca/"
...
outputPrefix <- "Bs_tissues_mollusca_DESeq2"

# Pull in Berghia gene ids for Mollusca-specific genes from kinfin analysis
mollusca_novel_genes <- scan("Mollusca_only_genes.txt", what="", sep="\n")

# Subset results to only include Mollusca-specific genes and order results by padj value (most significant to least)
res <- resdata[resdata$gene %in% mollusca_novel_genes, ]
res <- res[order(res$padj),]

# Send subsetted and normalized counts to tab delimited file
dds_filt <- dds[mollusca_novel_genes, ]
...
library("gplots")
distsRL <- dist(t(assay(rld[mollusca_novel_genes, ])))
...
rv <- rowVars(assay(rld[mollusca_novel_genes, ]))
...
heatmap.2(assay(rld[mollusca_novel_genes, ]), col=my_palette,
...
# Pull out annotation data for tissue-specific Mollusca genes
### Distal Ceras Only ###
