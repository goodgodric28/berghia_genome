####################################
# Bs_genome_DESeq2_all_others.R
# Written by: Jessica A. Goodheart
# Last Updated: 30 May 2023
# Purpose: To analyze differential expression data from Berghia tissues from all other genes.
...
directory <- "[PATH TO]/tissue_diffexp/deseq2_clades/all_others/"
...
outputPrefix <- "Bs_tissues_all_others_DESeq2"

# Pull in Berghia gene ids for All Other genes from kinfin analysis
all_others_novel_genes <- scan("All_other_genes.txt", what="", sep="\n")

# Subset results to only include All Other genes and order results by padj value (most significant to least)
res <- resdata[resdata$gene %in% all_others_novel_genes, ]
res <- res[order(res$padj),]

# Send subsetted and normalized counts to tab delimited file
dds_filt <- dds[all_others_novel_genes, ]
...
library("gplots")
distsRL <- dist(t(assay(rld[all_others_novel_genes, ])))
...
rv <- rowVars(assay(rld[all_others_novel_genes, ]))
...
select <- order(rowMeans(counts(dds_filt,normalized=T)),decreasing=T)[1:1000]
my_palette <- colorRampPalette(c("blue",'white','red'))(n=1000)
png(file=paste0(outputPrefix, "-HEATMAP-clade-expression.png"), width=1000, height=1000)
heatmap.2(assay(rld[all_others_novel_genes, ])[select,], col=my_palette,
          scale="row", key=T, keysize=1, symkey=T,
          density.info="none", trace="none",
          cexCol=0.6, labRow=F,
          main="Clade-specific Expression Heatmap - Top 1000 Genes")
dev.off()
