...
# Purpose: To analyze differential expression data from Berghia tissues in genes specific to Berghia.
...
directory <- "[PATH TO]/tissue_diffexp/deseq2_clades/berghia/"
...
outputPrefix <- "Bs_tissues_berghia_DESeq2"

# Pull in Berghia gene ids for Berghia-specific genes from kinfin analysis
berghia_novel_genes <- scan("Berghia_stephanieae_genes.txt", what="", sep="\n")

# Subset results to only include Berghia-specific genes and order results by padj value (most significant to least)
res <- resdata[resdata$gene %in% berghia_novel_genes, ]
...
library("gplots")
distsRL <- dist(t(assay(rld[berghia_novel_genes, ])))
...
rv <- rowVars(assay(rld[berghia_novel_genes, ]))
...
heatmap.2(assay(rld[berghia_novel_genes, ]), col=my_palette,
