####################################
# Bs_genome_DESeq2_tail.R
# Written by: Jessica A. Goodheart
# Last Updated: 3 August 2023
# Purpose: To analyze the clade distribution and expression of tail genes in Berghia
...
directory = "[PATH_TO]/tissue_diffexp/deseq2_tissues/tail/"
read_dir = "[PATH_TO]/tissue_diffexp/read_files/"
setwd(directory)

# Set the prefix for each output file name
outputPrefix = "Bs_tissues_tail_DESeq2"
...
dds = DESeq(ddsHTSeq)
res = results(dds, contrast=c(-1/6,-1/6,-1/6,-1/6,-1/6,-1/6,1))
...
# Subset results to only include upregulated tail genes 
...
# Subset results to only include downregulated tail genes 
...
other_genes = scan("[PATH_TO]/tissue_diffexp/deseq2/all_others/gene_annotations/Bs_tissues_all_others_DESeq2-TA-only-genelist.txt", what="", sep="\n")
mollusca_genes = scan("[PATH_TO]/tissue_diffexp/deseq2/mollusca/gene_annotations/Bs_tissues_mollusca_DESeq2-TA-only-genelist.txt", what="", sep="\n")
gastropoda_genes = scan("[PATH_TO]/tissue_diffexp/deseq2/gastropoda/gene_annotations/Bs_tissues_gastropoda_DESeq2-TA-only-genelist.txt", what="", sep="\n")
nudibranchia_genes = scan("[PATH_TO]/tissue_diffexp/deseq2/nudibranchia/gene_annotations/Bs_tissues_nudibranchia_DESeq2-TA-only-genelist.txt", what="", sep="\n")
aeolidina_genes = scan("[PATH_TO]/tissue_diffexp/deseq2/aeolidina/gene_annotations/Bs_tissues_aeolidina_DESeq2-TA-only-genelist.txt", what="", sep="\n")
berghia_genes = scan("[PATH_TO]/tissue_diffexp/deseq2/berghia/gene_annotations/Bs_tissues_berghia_DESeq2-TA-only-genelist.txt", what="", sep="\n")
...
row.names(out) = dat
colnames(out) = c("Only expressed in tail", "Upregulated in tail")
...
# Pull out genes both upregulated and exclusively expressed in the tail
